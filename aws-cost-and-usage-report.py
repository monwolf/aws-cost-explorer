...
def query_cost_explorer(client, days, months):
    ...
    if months:
        if months == 1:
            ...
            start = (now - datetime.timedelta(weeks=months*4)).strftime('%Y-%m-01')
    if days:
...
def main():
    ...
    parser.add_argument("--days", type=int, dest="days", default=0,
                        help="get data for daily usage and cost by given days.\
                        (Mutualy exclusive with 'months' option, default: 0)")
    parser.add_argument("--months", type=int, dest="months", default=1,
    ...
    out_line = ','.join(["Time Period", "Linked Account", "Service", "Amount", "Unit", "Estimated"])
    with open(output_fpath, "w") as out_fd:
        ...
        for result_by_time in data:
            ...
            for group in result_by_time['Groups']:
                ...
                out_line = "%s,%s,%s,%s,%s,%s\n" % (time_start, account_id, service, amount, unit, estimated)
                out_fd.write(out_line)
